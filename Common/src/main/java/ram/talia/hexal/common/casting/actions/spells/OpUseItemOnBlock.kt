...
object OpUseItemOnBlock : SpellAction {
    override val argc = 3
    override fun execute(
        ...
        return Triple(
                Spell(target, direction, item),
                MediaConstants.DUST_UNIT,
                listOf(ParticleSpray.burst(Vec3.atCenterOf(BlockPos(target)), 1.0))
        )
    }

    private data class Spell(val pos: BlockPos, val direction: Vec3, val item: ItemIota) : RenderedSpell {
