...
object OpSummonWisp : SpellOperator {
    ...
    override fun execute(args: List<SpellDatum<*>>, ctx: CastingContext): Triple<RenderedSpell, Int, List<ParticleSpray>>? {
        ...
        return Triple(
            Spell(pos, vel, hex.toList()),
            COST_SUMMON_WISP,
            listOf(ParticleSpray.burst(pos, 1.5), ParticleSpray.cloud(pos, 0.5))
        )
    }

    private data class Spell(val pos: Vec3, val vel: Vec3, val hex: List<SpellDatum<*>>) : RenderedSpell {
        override fun cast(ctx: CastingContext) {
            val colouriser = IXplatAbstractions.INSTANCE.getColorizer(ctx.caster)
            val wisp = ProjectileWisp(ctx.world, pos, ctx.caster)
